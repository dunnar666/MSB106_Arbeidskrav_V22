...
syssel_bos <- read_excel("Data/syssel_bos.xlsx")
...
```{r Arbeidssted_Hardanger_2_test, fig.cap="Hardanger/Ullensvang"}
library(lubridate)
...
syssel_bos %>% 
  filter(knr %in% c(4618)) %>% 
  ggplot(mapping = aes(x = År, y = `Alle Næringer`)) +
  geom_line(aes(group = knavn, colour = knavn)) +
  geom_hline(yintercept = 0) +
   labs(x = "År") +
  ggtitle("Fig. 14.5: Hardanger -> Ullensvang, sammenslått tidsserie 2008-2020") +
  theme(legend.position = 'bottom') +
  scale_color_hue(name = "Kommune")

# Denne gir en bedre fremstilling av Ullensvang kommune, der data for Odda og Ullensvang er plusset sammen for årene 2008-2019.
# Ved å benytte tall for slik sammenslått tidsserie vil problemer med plutselige hopp ved kommunesammenslåinger unngås.
# Slike hopp som oppstår i fig. 14, vil bidra til vanskeligheter med å tolke resultater.
```
